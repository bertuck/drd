...
export default class ApiOpenWeather {
  ...
  _map(forecastData, todayData, lang) {
    ...
    mapped.current = {
      ...
      date: todayData.dt ? formatDate(todayData.dt, lang) : ''
    };
  ...
  _mapForecast(daysData, lang) {
    ...
    for (var i=0; i<4; i++) {
      ...
      var dayDataFiltered = [];
      daysData.map(item => item.dt_txt.search(comingDays[i]) ? dayDataFiltered.push(item) : null);  //7 or 8 data objects represnting a day
      var dayMapped = {};
      dayMapped.date = dayDataFiltered[0] ? formatDate(dayDataFiltered[0].dt, lang) : "";  // Getting the date from the 1st data object (random)
      ...
      if (dayDataFiltered[dayDataFiltered.length / 2]) {
